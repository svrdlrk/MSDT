...
class TaskManager:
    def __init__(self, root):
        ...
        self.delete_button = tk.Button(self.frame, text="Удалить задачу", command=self.delete_task)
        self.delete_button.grid(row=5, column=0, pady=5)
        self.update_button = tk.Button(self.frame, text="Изменить задачу", command=self.update_task)
        self.update_button.grid(row=5, column=1, pady=5)
        self.complete_button = tk.Button(self.frame, text="Отметить как выполненную", command=self.mark_completed)
        self.complete_button.grid(row=6, column=0, pady=5)
        self.sort_button = tk.Button(self.frame, text="Сортировать по дате", command=self.sort_by_date)
        self.sort_button.grid(row=6, column=1, pady=5)
        self.export_button = tk.Button(self.frame, text="Экспорт в CSV", command=self.export_to_csv)
        self.export_button.grid(row=7, column=0, pady=5)
        self.import_button = tk.Button(self.frame, text="Импорт из CSV", command=self.import_from_csv)
        self.import_button.grid(row=7, column=1, pady=5)
        self.clear_button = tk.Button(self.frame, text="Очистить все задачи", command=self.clear_all_tasks)
